...
public class ItemHasherBase implements ItemHasher {
    ...
    public ItemHasherBase(final UserConnection connection, final MappingData mappingData) {
    ...
    public HashedItem toHashedItem(final Item item) {
        ...
        for (final StructuredData<?> data : item.dataContainer().data().values()) {
            ...
            final ItemDataComponentConverter.Result<?> result = this.itemComponentConverter.viaToMcStructs(data);
    ...
    private <T> int hash(final ItemDataComponentConverter.Result<T> result) {
        return result.type().getCodec().serialize(hashConverter(), result.value()).getOrThrow().asInt();
    }

    private int hashTag(final CompoundTag tag) {
        return ItemComponents_v1_21_5.V1_21_5.CUSTOM_DATA.getCodec().serialize(hashConverter(), tag).getOrThrow().asInt();
    }

    protected DataConverter<HashCode> hashConverter() {
        return HashConverter_v1_21_5.CRC32C;
    }
}
