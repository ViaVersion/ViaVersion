...
public class ViaDecodeHandler extends MessageToMessageDecoder<ByteBuf> implements ViaChannelHandler {
    ...
    protected void decode(final ChannelHandlerContext ctx, final ByteBuf buf, final List<Object> out) throws Exception {
        if (!connection.checkIncomingPacket(buf.readableBytes())) {
            throw CancelDecoderException.generate(null);
        }
        if (!connection.shouldTransformPacket(buf, connection.isClientSide() ? Direction.CLIENTBOUND : Direction.SERVERBOUND)) {
            out.add(buf.retain());
            return;
        }
